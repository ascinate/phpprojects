<?php
   ...
   //////////////////  For Cleaners //////////////////////
	...
	public function insertCleaner()
	{
	  ...
	  if($query->num_rows()=='')
	  {
		...
		if($result)
		{
			...
			$this->db->insert('cleaner_experience_master', array('cleaner_id' => $last_id));
			$this->db->insert('cleaner_availability_master', array('cleaner_id' => $last_id));
	...
	public function deleteCleaner()
    {
	   ...
	   $query = $this->cleaner_model->delete_user($id);
	   	  ...
	   	  $this->db->where('cleaner_id', $id);
	   	  $this->db->delete('cleaner_availability_master');

	   	  $this->db->where('cleaner_id', $id);
 		  $this->db->delete('cleaner_experience_master');
	...
	public function insertSchedule()
	{
		$appoint = array(
				...
				'if_dog' => $this->input->post('if_dog'),
				'if_cat' => $this->input->post('if_cat'));
			
		$query = $this->user_model->insertappointment($appoint);
    ...
    {
	    $id = $this->uri->segment(3);
		$query = $this->user_model->get_home_data($id);
	...
	public function updateHome()
	{
		...
		$query = $this->user_model->update_home($id, $homedata);
		redirect('dashboard/schedules'); 
   }
   ...
   public function editschedule()
    {
	    $id = $this->uri->segment(3);
		$query = $this->user_model->get_schedule_data($id);
	...
	public function updateSchedule()
	{
		...
		$query = $this->user_model->update_schedule($id, $data);
		redirect('dashboard/schedules'); 
   }
